...
public class SMGPQueryMessage extends SMGPMessage {
	...
	public SMGPQueryMessage(Date time, int queryType, String queryCode) throws IllegalArgumentException {
		if (time == null)
			throw new IllegalArgumentException(String.valueOf(
					String.valueOf((new StringBuffer(String.valueOf(String.valueOf(SMGPConstant.QUERY_INPUT_ERROR))))
							.append(":Time ").append(SMGPConstant.STRING_NULL))));
		if (queryType < 0 || queryType > 255)
			throw new IllegalArgumentException(String.valueOf(
					String.valueOf((new StringBuffer(String.valueOf(String.valueOf(SMGPConstant.QUERY_INPUT_ERROR))))
							.append(":QueryType ").append(SMGPConstant.INT_SCOPE_ERROR))));
		if (queryCode != null && queryCode.length() > 10)
			throw new IllegalArgumentException(String.valueOf(
					String.valueOf((new StringBuffer(String.valueOf(String.valueOf(SMGPConstant.QUERY_INPUT_ERROR))))
							.append(":QueryCode ").append(SMGPConstant.STRING_LENGTH_GREAT).append("10"))));
		int len = 31;
		buf = new byte[len];
