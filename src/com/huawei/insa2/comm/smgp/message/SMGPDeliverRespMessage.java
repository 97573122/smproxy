	...
	public SMGPDeliverRespMessage(byte msg_Id[], int status) throws IllegalArgumentException {
		if (msg_Id.length > 10)
			throw new IllegalArgumentException(String.valueOf(String
					.valueOf((new StringBuffer(String.valueOf(String.valueOf(SMGPConstant.DELIVER_REPINPUT_ERROR))))
							.append(":msg_Id").append(SMGPConstant.STRING_LENGTH_GREAT).append("10"))));
		if (status < 0) {
			throw new IllegalArgumentException(
					String.valueOf(String.valueOf(SMGPConstant.DELIVER_REPINPUT_ERROR)).concat(":result"));
		} else {
			int len = 26;
			buf = new byte[len];
