...
public class SMPPLoginMessage extends SMPPMessage {
	...
	public SMPPLoginMessage(int logintype, String systemId, String password, String systemType, byte interfaceVersion,
			byte addrTon, byte addrNpi, String addressRange) throws IllegalArgumentException {
		if (logintype != 1 && logintype != 2)
			throw new IllegalArgumentException(String.valueOf(
					String.valueOf((new StringBuffer(String.valueOf(String.valueOf(SMPPConstant.CONNECT_INPUT_ERROR))))
							.append(":loginCommandId ").append(SMPPConstant.OTHER_ERROR))));
		loginCommandId = logintype;
		if (systemId.length() > 15)
			throw new IllegalArgumentException(String.valueOf(
					String.valueOf((new StringBuffer(String.valueOf(String.valueOf(SMPPConstant.CONNECT_INPUT_ERROR))))
							.append(":systemId ").append(SMPPConstant.STRING_LENGTH_GREAT).append("15"))));
		if (password.length() > 8)
			throw new IllegalArgumentException(String.valueOf(
					String.valueOf((new StringBuffer(String.valueOf(String.valueOf(SMPPConstant.CONNECT_INPUT_ERROR))))
							.append(":password ").append(SMPPConstant.STRING_LENGTH_GREAT).append("8"))));
		if (systemType.length() > 12)
			throw new IllegalArgumentException(String.valueOf(
					String.valueOf((new StringBuffer(String.valueOf(String.valueOf(SMPPConstant.CONNECT_INPUT_ERROR))))
							.append(":systemType ").append(SMPPConstant.STRING_LENGTH_GREAT).append("12"))));
		if (addressRange.length() > 40) {
			throw new IllegalArgumentException(String.valueOf(
					String.valueOf((new StringBuffer(String.valueOf(String.valueOf(SMPPConstant.CONNECT_INPUT_ERROR))))
							.append(":addressRange ").append(SMPPConstant.STRING_LENGTH_GREAT).append("40"))));
		} else {
