...
public class SMPPSubmitMessage extends SMPPMessage {
	...
	public SMPPSubmitMessage(String serviceType, byte sourceAddrTon, byte sourceAddrNpi, String sourceAddr,
			byte destAddrTon, byte destAddrNpi, String destinationAddr, byte esmClass, byte protocolId,
			byte priorityFlag, String scheduleDeliveryTime, String validityPeriod, byte registeredDelivery,
			byte replaceIfPresentFlag, byte dataCoding, byte smDefaultMsgId, byte smLength, String shortMessage)
					throws IllegalArgumentException {
		if (serviceType.length() > 5)
			throw new IllegalArgumentException(String.valueOf(
					String.valueOf((new StringBuffer(String.valueOf(String.valueOf(SMPPConstant.SUBMIT_INPUT_ERROR))))
							.append(":serviceType ").append(SMPPConstant.STRING_LENGTH_GREAT).append("5"))));
		if (sourceAddr.length() > 20)
			throw new IllegalArgumentException(String.valueOf(
					String.valueOf((new StringBuffer(String.valueOf(String.valueOf(SMPPConstant.SUBMIT_INPUT_ERROR))))
							.append(":sourceAddr ").append(SMPPConstant.STRING_LENGTH_GREAT).append("20"))));
		if (destinationAddr == null)
			throw new IllegalArgumentException(String.valueOf(
					String.valueOf((new StringBuffer(String.valueOf(String.valueOf(SMPPConstant.SUBMIT_INPUT_ERROR))))
							.append(":destinationAddr ").append(SMPPConstant.STRING_NULL))));
		if (destinationAddr.length() > 20)
			throw new IllegalArgumentException(String.valueOf(
					String.valueOf((new StringBuffer(String.valueOf(String.valueOf(SMPPConstant.SUBMIT_INPUT_ERROR))))
							.append(":destinationAddr ").append(SMPPConstant.STRING_LENGTH_GREAT).append("20"))));
		if (scheduleDeliveryTime.length() != 0 && scheduleDeliveryTime.length() != 16)
			throw new IllegalArgumentException(String.valueOf(
					String.valueOf((new StringBuffer(String.valueOf(String.valueOf(SMPPConstant.SUBMIT_INPUT_ERROR))))
							.append(":scheduleDeliveryTime ").append(SMPPConstant.STRING_LENGTH_NOTEQUAL)
							.append("16"))));
		if (validityPeriod.length() != 0 && validityPeriod.length() != 16)
			throw new IllegalArgumentException(String.valueOf(
					String.valueOf((new StringBuffer(String.valueOf(String.valueOf(SMPPConstant.SUBMIT_INPUT_ERROR))))
							.append(":validityPeriod ").append(SMPPConstant.STRING_LENGTH_NOTEQUAL).append("16"))));
		if (smLength > 254)
			throw new IllegalArgumentException(String.valueOf(
					String.valueOf((new StringBuffer(String.valueOf(String.valueOf(SMPPConstant.SUBMIT_INPUT_ERROR))))
							.append(":smLength ").append(SMPPConstant.STRING_LENGTH_GREAT).append("254"))));
		if (shortMessage.length() > 254) {
			throw new IllegalArgumentException(String.valueOf(
					String.valueOf((new StringBuffer(String.valueOf(String.valueOf(SMPPConstant.SUBMIT_INPUT_ERROR))))
							.append(":shortMessage ").append(SMPPConstant.STRING_LENGTH_GREAT).append("254"))));
		} else {
			int len = 33 + serviceType.length() + sourceAddr.length() + destinationAddr.length()
					+ scheduleDeliveryTime.length() + validityPeriod.length() + smLength;
			buf = new byte[len];
			setMsgLength(len);
			setCommandId(4);
			setStatus(0);
